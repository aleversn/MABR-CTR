...
class Preprocess():
    ...
    def check_sku_cid3(cls):
        ...
        i = 0
        for sku in list(micro_sku_id.keys()):
    ...
    def get_data_base_data_bh_dwell(cls):
        ...
        dict_bhStr_emb={}
        dict_dwellStr_emb={}
        for bhdwll_idInt in list(dict_bhdwll_idInt_itemStr.keys()):
    ...
    def get_item_raw_from_session_data(cls, folder, file_in, min_cnt, max_cnt):
        ...
        ftime = open(FileTool.add_folder(folder, 'time.raw'), 'w')
    ...
    def get_embedding_item(cls, folder, file_in, file_out, size, min_count=0):
        ...
        cmd = "/home/cwt/WSDM2020_HUP-master/HUP_code/word2vec/word2vec -train %s -output %s -size %s -min-count %s" % (file_in, file_out, str(size), str(min_count))

        # ProTool.exec_cmd(cmd)
        print("Cmd:%s" % cmd)

        w2v_model = Word2Vec(LineSentence(file_in), vector_size=size, min_count=min_count)
        #w2v_model = Word2Vec(LineSentence(file_in), size=size, min_count=0)
        w2v_model.wv.save_word2vec_format(file_out, binary=False)
    ...
    def get_embedding_items(cls):
        ...
        Preprocess.get_embedding_item(Config.folder, "time.raw", "time.w2v", 30)
    ...
    # transfer xx.w2v to xx.mapping and xx.reidx
    @classmethod
    def get_data_item_mapping_items(cls):
        print("get_data_item_mapping_items...")
        # Config.micro_item_list = ["sku", "bh", "cid3", "dwell", "gap"]
    ...
    def get_data_session_train_filt_data_to_itemid(cls, file_in, mode):
        ...
        micro_item2vec = Data.load_micro_item_vec_mode(mode)  # return item2vec from 5 .w2v
        micro_item2id = Data.load_micro_itemInt_idInt(micro_item_list)  # return item2it from 5 .map
    ...
    def get_file_to_id_mapping(cls, folder, file_in, file_out, file_out_mapping, sep=' '):
        # file_in: session.SBCGD, file_out: session.SBCGD.id, file_out_mapping: session.SBCGD.id.mapping
        file_in = FileTool.add_folder(folder, file_in)
        file_out = FileTool.add_folder(folder, file_out)
    ...
    def build_raw_data(cls):
        ...
        print("processing %s..." % Config.get_exp_label()) # Computers or Applicances
        # get_topsku() and get_lines_min_cnt() are filtering out the items with fewer than 30/40 occurrences
        # get topsku, JD_xxx.topsku,
        Preprocess.get_topsku(Config.folder, Config.file_data_raw, Config.topsku,
                              Config.file_data_raw_topsku, Config.min_cnt_sku_limit)
        # from JD_xxx.topsku get JD_xxx.topsku.lenxx
        ...
        # from JD_xxx.topsku.lenxx to xx.raw
        Preprocess.get_item_raw_from_session_data(Config.folder, Config.file_data_raw_topsku_len,
                                                  Config.min_cnt_line_items, Config.max_cnt_line_items)
        # from sku.raw to sku.uniq
        Preprocess.filt_item_raw_to_uniq(Config.folder)
        Preprocess.get_sku_cate_num(Config.folder)
        # from dwell.raw/gap.raw to dwell.id/gap.id
    ...
    @classmethod
    def get_sku_cate_num(cls, folder):
        FileTool.func_begin('get_sku_cate_num')
        file_sku = FileTool.add_folder(folder, 'sku.uniq')
        file_cate = FileTool.add_folder(folder, 'cid3.raw')
        for file in [file_sku, file_cate]:
            listlist = FileTool.read_file_to_list_list(file)
            dict = {}
            for list in listlist:
                for item in list:
                    StructureTool.addDict(dict, item)
            print('number of {} is : {}'.format(file, len(dict)))
        FileTool.func_end('get_sku_cate_num')
    ...
    def get_topsku(cls, folder, file_in, file_out_topsku, file_out_lines, min_sku_cnt):
        ...
        listlist = FileTool.read_file_to_list_list(os.path.join(folder, file_in), Config.file_sep) # file_sep = ' '

        dict_sku = {}
        for line in listlist:  # a line is a user's all session
            for unit in line:  # a unit is a session
                item = SessionItemBase(unit)
                sku = item.sku
                StructureTool.addDict(dict_sku, sku)  # sku's times in dataset

        print("dict_sku:", len(dict_sku))  # sku num in total

        valid_sku=set()  # valid_sku: Number of sku appearances > min_sku_cnt:50
        for sku in dict_sku:
        ...
        print("valid_sku: ", len(valid_sku))  # total num of valid_sku
        FileTool.write_file_listStr(os.path.join(folder, file_out_topsku), valid_sku_list) # file_out to file topsku

        valid_lines = []  # valid_lines: a user's all session is valid_sku
        bh_counts = 0
        for line in listlist:
            flag=True
            if(Config.get_exp_label() != "tianchi"):
        ...
        print("valid_lines: ", len(valid_lines))  # total num of valid user
        print("valid_behaviors: ", bh_counts)
        FileTool.write_file_list_list(os.path.join(folder, file_out_lines), valid_lines, Config.file_sep)
        # file out to JD_Computers.topsku / JD_Applicances.topsku
