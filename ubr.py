...
class UBRBase(object):
    ...
    def build_index_and_loss(self, probs):
        uniform = tf.random_uniform(tf.shape(probs), 0, 1)  # tf.random_uniform: 用于从均匀分布中输出随机值
        ...
        tf.summary.scalar('ubr_reward', self.reward)    # 添加变量到直方图中
    ...
    def build_optimizer(self):    
        ...
        for grad, var in gvs:
            if grad is not None:
                capped_gvs.append((tf.clip_by_norm(grad, 5.), var)) # tf.clip_by_norm: 梯度裁剪
